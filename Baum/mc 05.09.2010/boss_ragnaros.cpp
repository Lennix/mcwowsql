...
struct MANGOS_DLL_DECL boss_ragnarosAI : public ScriptedAI
{
	...
	// Für CasterKick relevant, wenn der Feuerball einschlägt = Kick
    ...
    void UpdateAI(const uint32 diff)
    {
		...
		if (CasterKick_Timer < diff)
		{
			...
			{
				m_creature->CastSpell(pCaster,SPELL_FIREBALL,true);			// Feuerball löst den Spellhit für Kick aus
        ...
        if (m_creature->IsWithinDistInMap(m_creature->getVictim(), ATTACK_DISTANCE))
        {
            ...
            {	
				m_creature->getVictim()->SetArmor(0);							//feueresi wird benötigt
        ...
        else
        {
			...
			if (SecondTankFound == true)
			{
				...
				if (m_creature->isAttackReady() && !m_creature->IsNonMeleeSpellCasted(false))
				{
					sTank->SetArmor(0);						//feueresi wird benötigt
