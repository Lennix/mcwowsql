...
// Son of Hakkar <- Hackfix, We need support for caster mobs wich are dead
struct MANGOS_DLL_DECL npc_sonofhakkarAI : public ScriptedAI
{
	npc_sonofhakkarAI(Creature* pCreature) : ScriptedAI(pCreature)
    {
        Reset();
    }
	...
	bool PoisonCloud_Status;

	void Reset()
    {
		PoisonCloud_Status	= true;
	}

	void UpdateAI(const uint32 uiDiff)
    {
		if (m_creature->GetHealthPercent() < 3.0f && PoisonCloud_Status)
		{
			DoCastSpellIfCan(m_creature->getVictim(),SPELL_POISONCLOUD);
			PoisonCloud_Status = false;
		}
...
CreatureAI* GetAI_npc_sonofhakkar(Creature* pCreature)
{
    return new npc_sonofhakkarAI(pCreature);
}
...
void AddSC_instance_zulgurub()
{
    ...
    newscript = new Script;
    newscript->Name = "npc_sonofhakkar";
    newscript->GetAI = &GetAI_npc_sonofhakkar;
    newscript->RegisterSelf();
}
